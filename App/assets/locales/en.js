var locale = {
    goodmorning : "Good morning",
    goodafternoon : "Good afternoon",
    goodevening : "Good evening",
    goodnight : "Good night",
    goodmidnight : "Enjoy the midnight",
    thunderstorm : "Thunderstorm",
    rainy : "Rainy",
    snow : "Snowing",
    cloudy : "Cloudy",
    clearsky : "Clear sky",
    fewclouds : "A little cloudy",
    scatteredclouds : "Somewhat cloudy",
    veryrain : "Heavy rain",
    snowrain : "Snowing & raining",
    mist : "Mist",
    feel : "a nice",
    peaceful : "a peaceful",
    freezing : "a freezing",
    cold : "a cold",
    mild : "a mild",
    hot : "a hot",
    extremelyhot : "an extremely hot",
    its : "It's",
    morningin : "morning in",
    noonin : "noon in",
    afternoonin : "afternoon in",
    eveningin : "evening in",
    nightin : "night in",
    midnightin : "midnight in",
    lastupdated : "Last updated",
    undo: "Undo",
    redo: "Redo",
    insert: "Insert",
    search: "Search",
    cut: "Cut",
    copy: "Copy",
    select_all: "Select All",
    bold: "Bold",
    italic: "Italic",
    underline: "Underlined",
    strikethrough: "Strikethrough",
    view_profile: "View profile",
    open_new: "Open new",
    create_new: "Create new",
    unsaved_changes: "Unsaved changes",
    unsaved_changes_longer: "You have unsaved changes",
    binned_note: "Note is in bin",
    hide_subheader: "Hide subheader",
    show_subheader: "Show subheader",
    actions: "Actions",
    share: "Share",
    duplicate: "Duplicate",
    delete: "Delete",
    properties: "Properties",
    close: "Close",
    save: "Save",
    note_history: "Note History",
    recover: "Recover",
    recover_all: "Recover All",
    perm_delete: "Permamently Delete",
    perm_delete_all: "Permamently Delete All",
    couldnt_connect: "Couldn't connect to server",
    wrong_response: "Server returned invalid response",
    retry: "Retry",
    status: "Status",
    click_expand: "Click to expand more",
    click_shrink: "Click to hide",
    display: "Display",
    view: "View",
    grid: "Grid",
    list: "List",
    sort: "Sort",
    name: "Name",
    size: "Size",
    type: "Type",
    edit: "Edit",
    new_folder: "New folder",
    bin: "Bin",
    date_opened: "Date opened",
    date_modified: "Date modified",
    date_created: "Date created",
    welcome_to: "Welcome to your new",
    welcome_back: "Welcome back to your",
    you: "You",
    text_editor: "text editor",
    photo_editor: "photo editor",
    calculator: "calculator",
    audio_player: "audio player",
    notes_app: "notes app",
    logged_out: "Logged out",
    password: "Password",
    sign_in: "Sign In",
    caps_enabled: "Caps Lock is enabled",
    wrong_password: "Wrong password",
    empty: "Empty",
    at_least_change: "At least change it",
    sign_in_qr: "Sign In with QR",
    open_notes: "Open notes",
    username: "Username",
    username_too_long: "Username is too long",
    if_new: "Don't have an account?",
    create_account: "Create an account",
    already_account: "Already have an account?",
    sign_up: "Sign Up",
    email: "Email",
    starts_with_whitespace: "Your password starts with space",
    ends_with_whitespace: "Your password ends with space",
    incorrect_username_password: "Incorrect username or password",
    server_shutting_down: "Server shutdown in 30 seconds",
    save_wait: "Save everything and wait",
    save_exit: "Save everything and exit",
    dont_save_wait: "Don't save and wait",
    offline: "Offline",
    online: "Online",
    note: "Note",
    note_description: "Text with media",
    calculator: "Calculator",
    calculator_description: "Math optimized workspace",
    webapp: "Web App",
    webapp_description: "Site builder",
    presentation: "Presentation",
    presentation_description: "Slides environment",
    create_new_project: "Create a new project",
    present: "Present",
    select_device_remote_control: "Select a device for remote control",
    none: "None",
    success_set_remote: "Remote device responded successfully",
    add_slide: "Add slide",
    presenting_on: "Presenting on",
    slide: "Slide",
    next_slide: "Next slide",
    previous_slide: "Prevous slide",
};